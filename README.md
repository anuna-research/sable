...
4. **ZK proof of liveness** -- Twelve quantized delta fingerprints, the per-round coverage counts, convexity scores and glint fingerprints enter the Halo2 circuit as private witnesses. SPEC-008 also adds twelve private rolling-shutter row-band symbols, one global phase and two frame-timing deltas. Every public liveness parameter -- the expected colors and temporal waveform, every threshold and floor, and SHA-256 of both coin-flip nonces -- is hashed with Poseidon into one **challenge digest** the verifier recomputes, so a proof answers exactly one challenge under exactly the parameters the verifier issued. The temporal relation remains disabled unless a verifier-owned protected capture validator authorizes it; the web demo has no such validator and offers observe-only diagnostics. The circuit outputs a single liveness bit. A proof-only recipient sees the public result and digest; the demo server also receives the images and extracts the reflectance data.
